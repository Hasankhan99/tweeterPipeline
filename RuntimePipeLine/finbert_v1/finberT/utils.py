...
class FinSentProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ["Increase above 2%", "Decrease below 2%", "neutral"]
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, mode='classification'):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if mode == 'classification':
            ...
            raise ValueError(
                "The mode should either be classification or regression. You entered: " + mode)
...
def get_prediction(text, model, tokenizer):
    ...
    model.eval()
    prediction = softmax(
        model(all_input_ids, all_attention_mask, all_token_type_ids).detach().numpy())
    return prediction
