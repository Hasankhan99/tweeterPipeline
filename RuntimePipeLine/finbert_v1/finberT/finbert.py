    ...
    def get_data(self, phase):
        ...
        if phase == 'train':
            train = pd.read_csv(os.path.join(
                self.config.data_dir, 'train.csv'),  index_col=False)
            weights = list()
...
def predict(text, model, write_to_csv=False, path=None, use_gpu=False, gpu_name='cuda:0', batch_size=5):
    ...
    label_list = ['Increase above 2%', 'Decrease below 2%', 'neutral']
    label_dict = {0: 'Increase above 2%', 1: 'Decrease below 2%', 2: 'neutral'}
    ...
    for batch in chunks(sentences, batch_size):
        ...
        with torch.no_grad():
            ...
            batch_result = {'sentence': batch,
                            ...
                            'expect_return_probability': sentiment_score}
